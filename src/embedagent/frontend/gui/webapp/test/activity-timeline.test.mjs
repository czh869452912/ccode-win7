...
} from "../src/session-runtime/timeline/activity-timeline.js";
import { ACTIVITY_ROW_KINDS } from "../src/session-runtime/timeline/activity-types.js";
import {
  buildChangedFilesTree,
  summarizeChangedFiles,
  summarizeDiffStats,
} from "../src/session-runtime/timeline/diff-activity.js";
import { buildWorkPresentation } from "../src/session-runtime/timeline/tool-activity.js";

export function runActivityTimelineTests() {
  {
  ...
  const settledRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-1",
  ...
  const timedFoldRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-timed",
        userItem: {
  ...
  assert.equal(timedFoldRows[1].kind, ACTIVITY_ROW_KINDS.TURN_FOLD);
  ...
  const runningRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-live",
  ...
  const grepRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  const metadataRows = buildActivityTimelineRows({
    toolCatalog: {
      pytest: {
        name: "pytest",
        label: "Pytest",
  ...
  const catalogDrivenRows = buildActivityTimelineRows({
    toolCatalog: {
      bash: {
        name: "bash",
        label: "Shell",
  ...
  const undeclaredBuiltInRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  const actionPresentationRows = buildActivityTimelineRows({
    turnGroups: [
      {
        ...
        steps: [
          {
            ...
            activityItems: [
              ...
              },
              {
                ...
                toolTitle: "EmbedAgent · preview_status",
  ...
  const actionRows = actionPresentationRows.filter((row) => row.kind === ACTIVITY_ROW_KINDS.WORK);
  ...
  assert.equal(mcpPresentation.heading, "EmbedAgent · preview_status");
  ...
  const detailRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-detail",
  ...
  const detailWorkRows = detailRows.filter((row) => row.kind === ACTIVITY_ROW_KINDS.WORK);
  ...
  const catalogDiffRows = buildActivityTimelineRows({
    toolCatalog: {
      write_file: {
        name: "write_file",
        metadata: { changed_path_arg: "path" },
  ...
  const catalogDiffSummary = catalogDiffRows.find((row) => row.kind === ACTIVITY_ROW_KINDS.DIFF_SUMMARY);
  assert.deepEqual(catalogDiffSummary.files.map((file) => file.path), ["src/catalog-row.c"]);

  const undeclaredDiffRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  assert.equal(undeclaredDiffRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.DIFF_SUMMARY), false);

  const interruptedRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  const systemRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-system",
  ...
  const experienceRows = buildActivityTimelineRows({
    turnGroups: [],
    currentStatus: "idle",
    turnExperience: {
      status: "blocked",
  ...
  const reviewNameOnlyRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  assert.equal(reviewNameOnlyRow.kind, ACTIVITY_ROW_KINDS.COMMAND_RESULT);
  assert.equal(reviewNameOnlyRow.label, "");
  assert.equal(reviewNameOnlyRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.REVIEW_RESULT), false);

  const richRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-rich",
  ...
  const richFold = richRows.find((row) => row.kind === ACTIVITY_ROW_KINDS.TURN_FOLD);
  assert.ok(richFold);
  assert.equal(richFold.workCount, 1);
  assert.equal(richFold.reasoningCount, 1);
  assert.deepEqual(
    richFold.entries.map((entry) => entry.kind),
    [ACTIVITY_ROW_KINDS.CONTEXT_SUMMARY, "reasoning", ACTIVITY_ROW_KINDS.WORK],
  );
  assert.equal(richFold.entries[0].content, "older turns summarized");
  assert.equal(richFold.entries[1].wordCount, 6);
  assert.equal(richRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.COMMAND_RESULT), true);
  const reviewRow = richRows.find((row) => row.kind === ACTIVITY_ROW_KINDS.REVIEW_RESULT);
  assert.ok(reviewRow);
  ...
  const thinkingRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  assert.equal(thinkingRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.WORKING), true);
  const thinkingRow = thinkingRows.find((row) => row.kind === ACTIVITY_ROW_KINDS.WORKING);
  assert.equal(thinkingRow.turnId, "turn-thinking");
  assert.equal(thinkingRow.label, undefined);

  const streamingReasoningRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-stream",
  ...
  assert.equal(streamingReasoningRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.WORKING), true);

  const priorReasoningActiveThinkingRows = buildActivityTimelineRows({
    turnGroups: [
      {
        turnId: "turn-prior",
  ...
  assert.equal(priorReasoningActiveThinkingRows.some((row) => row.kind === ACTIVITY_ROW_KINDS.WORKING), true);

  const interleavedRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  const multiCycleRows = buildActivityTimelineRows({
    turnGroups: [
      {
  ...
  const settledInterleavedRows = buildActivityTimelineRows({
    turnGroups: [
      {
