...
- Official GUI static asset policy: generated files under
  ...
  changes.
- Official GUI terminal boundary: the terminal bottom drawer is a GUI app-shell hosted, thread-scoped surface implemented with Windows 7-compatible Python stdlib subprocess pipes. Bottom drawer terminal new/select actions and terminal id generation are owned by `webapp/src/app-runtime/terminal-controller.js`; `App.jsx` must not import `nextTerminalId` or dispatch `terminal_active_set` inline. It is not a full PTY, does not add runtime dependencies, and does not write transcript history, telemetry, workflow state, source-control checkpoints, or Agent Core policy.
