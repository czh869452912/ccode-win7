...
- Official workflow prompt boundary: `PromptAssemblyService` owns workflow prompt append/dedupe and emits generic `workflow_prompt` system messages. Legacy `harness_prompt` messages are accepted only for historical session/transcript dedupe compatibility.
