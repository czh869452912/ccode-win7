...
- `embedagent-core` owns the workflow-neutral `embedagent_core` SDK. It has no
  ...
  workflow packages.
- `embedagent-protocol` owns the stdlib-only `embedagent_protocol` DTOs consumed
  across Host and replaceable UI shells.
- `embedagent-host` owns the generic hosted runtime, concrete local services,
  providers, tools, stores, and session hosting. It depends only on exact-matched
...
The distribution dependency direction is:
...
The builder removes only known generated artifacts. For an external
...
- Official GUI workspace lifecycle boundary: app bootstrap, workspace open/activate/remove, and workspace-path input updates are owned by `webapp/src/app-runtime/workspace-controller.js`; `App.jsx` wires controller methods directly and must not inline `workspace_path_changed` dispatches or workspace lifecycle HTTP calls.
- Official GUI session-list loading boundary: renderer session list loading is owned by `webapp/src/app-runtime/session-list-controller.js`; `App.jsx` composes that controller handle and must not define a `loadSessions` wrapper, directly fetch `/api/sessions`, or dispatch `sessions_loaded`.
- Official GUI session-activation boundary: session bootstrap activation is owned by `webapp/src/app-runtime/session-activation-controller.js`; `App.jsx` composes the controller as the `loadSession` handle and must not define an inline `loadSession` wrapper or derive activation payloads directly.
...
- Official GUI interaction response busy-state bridge: pending response request
  ...
  inline request-id normalization helpers. Interaction response submission and
  response `log_event` emission live in
  `webapp/src/app-runtime/interaction-response-controller.js`; `App.jsx` must
  not inject a root-level `logEvent` callback.
- Official GUI active-workspace data boundary: the post-activation read-model refresh fanout for sessions, session capabilities, workspace files, and local status surfaces is owned by `webapp/src/app-runtime/active-workspace-data-loader.js`; `App.jsx` wires dependencies but must not inline that reload `Promise.all` or wrap Source Control status refresh in a root-level forwarding lambda.
...
- Official GUI static asset policy: generated files under
  ...
  changes.
- Official GUI terminal boundary: the terminal bottom drawer is a GUI app-shell hosted, thread-scoped surface implemented with Windows 7-compatible Python stdlib subprocess pipes. Bottom drawer terminal new/select actions, terminal id generation, and right-panel active terminal pane new/split/select/close actions are owned by `webapp/src/app-runtime/terminal-controller.js`; `App.jsx` must not import `nextTerminalId`, dispatch `terminal_active_set` inline, or pass `activeRightPanelSurface` through inline terminal callbacks. It is not a full PTY, does not add runtime dependencies, and does not write transcript history, telemetry, workflow state, source-control checkpoints, or Agent Core policy.
...
- Official GUI right-panel tab lifecycle boundary: tab activate, close, close-others, close-to-right, close-all, add-surface, and Files-surface opening are owned by `webapp/src/app-runtime/right-panel-controller.js`; active surface lookup is owned by `webapp/src/workbench/surfaces.js` selectors. `App.jsx` must wire controller methods directly instead of inline right-panel tab reducer dispatch blocks, and must not resolve active surfaces with root-level `surfaces.find(...)` logic.
- Official GUI surface-panel action boundary: generic SurfacePanel actions for diff-file focus, Source Control refresh/file selection, and app-shell settings patching are owned by `webapp/src/app-runtime/surface-panel-controller.js`; `webapp/src/app-runtime/surface-panel-props.js` maps state/chrome/controller handles into `SurfacePanel` props. `App.jsx` wires that props builder and must not inline-dispatch `diff_file_focused`, `app_shell_settings_changed`, or per-action `surfacePanelController.*` prop mappings.
...
- `packages/embedagent-core/src/embedagent_core/turn_snapshot_service.py`
...
- `packages/embedagent-core/src/embedagent_core/ports.py`,
  `packages/embedagent-core/src/embedagent_core/policies.py`, and
...
- `packages/embedagent-core/src/embedagent_core/compaction_journal.py`
...
- `packages/embedagent-core/src/embedagent_core/turn_experience.py`
...
- `packages/embedagent-host/src/embedagent_host/hosted_command_service.py` and
...
- `src/embedagent/core/` and `packages/embedagent-protocol/src/embedagent_protocol/`
  Product frontend facade and standalone wire DTO contracts.
