...
- GUI `ProtocolAdapter` / `ClientRuntime` 与 TUI `TerminalRuntime` 已成为各自唯一 effect owner；strict DTO path 只产出 `snake_case` version 1 bootstrap 和 canonical envelope，直接 endpoint helper、三参数 event callback 及 TUI session/timeline/workspace services 已删除。当前前端阻塞转为 GUI/TUI 尚未消费同一 product-compiled shell descriptor，TUI 仍使用固定 command/surface catalog。
...
1. 先完成 GUI/TUI 共享 shell descriptor 注册，再收敛 Pi 式最小体验并清除应用语义展开字段。
