...
- CLI 与 TUI 共用 Python `SessionClientRuntime`；GUI 使用 browser-only JavaScript `SessionClientRuntime`。两种实现通过同一 JSON fixture 验证 activation、returned-bootstrap transaction、cursor、recovery、interaction、terminal evidence、generation、close 和 reentrant dispatch。所有 bootstrap-producing operation 都把 request、install、`session_activated` dispatch、recovery、rollback 与 ordered buffer drain 保持在一个 runtime transaction 中；controller 不拥有 bootstrap 安装，frontend runtime 不拥有 durable session truth。
