...
- 当前收敛：命令 palette、right-panel add-surface launcher、bottom drawer tab 与 keybinding target 现在全部按 `/api/app/bootstrap` 的 `app_commands`、`workspace_commands`、`surfaces.right_panel` 和 `surfaces.bottom_drawer` descriptor records 过滤/合并；renderer-local surface registry 只保留已支持 renderer 的 mounting metadata，并且不再导出固定 surface id list 给调用方当能力真相，缺失整个 `capabilities` 对象或相关 descriptor arrays 时都不再填充 GUI 默认入口。
