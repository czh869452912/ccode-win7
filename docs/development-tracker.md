...
> 更新日期：2026-06-04（GUI mode/task vocabulary closeout）
...
- 最新 capability extension contract：`ExtensionManager` 已从默认 C/C++ workflow extension 边界扩展为共享 in-process capability boundary，新增通用 extension diagnostics、resource discovery contract、context hook、tool-call/tool-result hooks、dynamic in-process tool registration 与 frontend snapshot diagnostics；project-local Python extension loading 和 reload command 仍是后续 slice。`
...
| 2026-06-04 | Capability extension contract Slice 1 落地：新增通用 extension diagnostics、resource discovery contract、context hook、tool-call/tool-result hooks 与 session snapshot diagnostics；project-local Python extension loading、dynamic tool registration 和 reload command 仍保持 deferred |
