...
- 当前收敛：Right-panel tab 激活副作用现在来自 renderer-supported surface definition 的 `activationKind` metadata，并通过 `right-panel-controller.js` 的 `RIGHT_PANEL_ACTIVATION_HANDLERS[definition.activationKind]` 查表执行；App 只委托 `rightPanelController.activateSurface(surface)`，不再导入 `surfaceDefinitionFor(...)`、检查 `definition.activationKind` 或直接调用 `terminalController.openSession(...)`。
- 当前收敛：App-level resource open flows 也不再持有具体 right-panel surface kind：文件、预览和 Files 浏览器打开分别委托 `rightPanelController.openFileSurface(...)`、`rightPanelController.openPreviewSurface(...)` 和 `rightPanelController.openFilesSurface()`；App 不再直接 dispatch `kind: "file"` / `kind: "preview"` 或调用 `openRightPanelSurface("files")`，文件预览 payload 也删除了旧的 `preview.kind = "file"` 冗余字段。
- 当前收敛：Right-panel semantic open methods now return whether the surface actually opened; App 的文件打开流程在 `openFileSurface(...)` 返回 false 时停止 `file_preview_load_started` 与 `/api/files/...` 内容加载，避免未声明 File Preview 的 agent 仍触发隐藏预览副作用。
- 当前收敛：Preview open / refresh / external-open flows now call `rightPanelController.canOpenPreviewSurface()` before invoking backend preview routes, so agents that do not declare the Preview surface do not trigger hidden preview service calls.
