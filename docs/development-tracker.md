...
- 最新 durable operation log 切片：`src/embedagent/session_operation_log.py` 已新增纯 `OperationLogReducer`，可从 transcript 事件还原 step/tool/explicit operation 的 started/finished/interrupted 状态；`SessionRestorer` 已暴露 `operation_state`，并只对已消费的有效 transcript 前缀做 reducer；`QueryEngine` 已在不替换旧 `tool_call/tool_result/loop_transition` 事件的前提下，附加写入 schema_v2 `operation_started/operation_finished/operation_interrupted` 生命周期事件。`
...
| T-029 | Pi-inspired minimal Core 第一阶段：durable operation log / reducer | `in_progress` | 已落地第一刀：新增 `OperationLogReducer`、`SessionRestoreResult.operation_state`，并让 `QueryEngine` 附加写入 schema_v2 operation lifecycle 事件；后续继续扩展 provider request、context snapshot、workflow patch 与 AgentKernel save point 语义 |
