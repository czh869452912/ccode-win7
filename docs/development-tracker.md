...
> 更新日期：2026-06-05（local resource reload Slice 3）
...
- 最新 self-extensible Agent Core：`ExtensionManager` 已从默认 C/C++ workflow extension 边界扩展为共享 in-process capability boundary，新增通用 extension diagnostics、resource discovery contract、context hook、tool-call/tool-result hooks、dynamic in-process tool registration 与 frontend snapshot diagnostics；`.embedagent/skills`、`.embedagent/prompts`、`.embedagent/recipes` 本地文件资源已可通过 runtime、adapter、slash command 与 GUI/core API reload，project-local Python extension loading 仍是后续受控 slice。`
...
| 2026-06-04 | Capability extension contract Slice 1 落地：新增通用 extension diagnostics、resource discovery contract、context hook、tool-call/tool-result hooks 与 session snapshot diagnostics；project-local Python extension loading 仍保持 deferred |
