...
- 当前收敛：命令 palette、right-panel add-surface launcher、bottom drawer tab 与 keybinding target 现在全部按 `/api/app/bootstrap` 的 `app_commands`、`workspace_commands`、`surfaces.right_panel`、`surfaces.bottom_drawer` 和 `keybindings` descriptor records 过滤/合并；renderer-local surface registry 只保留已支持 renderer 的 mounting metadata，并且不再导出固定 surface id list 给调用方当能力真相，缺失整个 `capabilities` 对象或相关 descriptor arrays 时都不再填充 GUI 默认入口。
...
- 当前收敛：有 active core 时，`/api/app/bootstrap` 会从 core 的安全 capability projection 带出 `agentApplication`、`agentApplications` 和 `emptyState`；NoWorkspaceState 优先使用 app-shell empty-state，因此 GUI 可在打开 session 前按当前基础/专用 agent 调整空状态文案。
