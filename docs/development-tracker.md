...
- 当前收敛：命令 palette、right-panel add-surface launcher、bottom drawer tab 与 keybinding target 现在全部按 `/api/app/bootstrap` 的 `app_commands`、`workspace_commands`、`surfaces.right_panel`、`surfaces.bottom_drawer` 和 `keybindings` descriptor records 过滤/合并；app/workspace command 的 label/order/visible_when/surface/drawer/keywords 也来自 backend descriptor，renderer-local command/surface registry 只保留已支持 renderer 的 mounting/execution metadata，并且不再导出固定 app command 或 surface id list 给调用方当能力真相，缺失整个 `capabilities` 对象或相关 descriptor arrays 时都不再填充 GUI 默认入口。
