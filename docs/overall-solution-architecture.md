...
### 3.4 文件系统和 SQLite 是第一性基础设施

在离线 Windows 7 环境下，最稳妥的基础设施是：

- 文件系统：规则、记忆、快照、日志、索引产物
- SQLite：会话、事件、权限、任务、结构化状态
...
- SQLite 存结构化状态。
- Markdown/文本文件存规则、长期记忆、任务摘要、项目约定。
- 支持审计、差异比对、恢复和人工干预。
...
writable_globs = ["docs/specs/**/*.md", "docs/tasks/**/*.md"]
...
writable_globs = ["src/**/*.c", "src/**/*.h", "include/**/*.h"]
...
2. LLM 工具调用：所有模式均有 `switch_mode(target: str)` 工具，调用后更新当前模式，以新模式 prompt 重建上下文继续（不推进循环）
...
- Working / Session：SQLite + 少量内存缓存
- Project / Archive：Markdown 文档 + SQLite 索引
