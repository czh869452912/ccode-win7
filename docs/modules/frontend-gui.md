...
- Tool/work row preview text and command/file request kind are catalog-driven. `t3-timeline.js` resolves the tool descriptor from session capabilities and uses safe `metadata.preview_arg` plus `permission_category`; it does not infer previews from built-in names such as `bash`, `read_file`, `grep_text`, or workflow-package tool names.
