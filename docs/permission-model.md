...
Local resource reload is a read/discovery operation and does not grant execution rights. Skills discovered from `.embedagent/skills` may be listed in system prompts or explicitly expanded through `/skill:<name> [args]`, but expansion only reads workspace-bound Markdown into a normal user turn and grants no additional tool or write permissions. Recipes discovered from `.embedagent/recipes/*.json` still execute through `run_recipe` and the same recipe/toolchain permission rules as bundled workspace recipes.
