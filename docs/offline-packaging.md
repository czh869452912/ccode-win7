...
| 项目 | 当前状态 | 备注 |
|------|----------|------|
| Python 运行时 | `missing` | 目标运行时仍未切到 Python 3.8 embeddable distribution |
...
| MinGit portable | `missing` | 运行时尚未随仓库存放 |
...
| 组件 | 目标位置 | 来源基线 | 当前状态 |
|------|----------|----------|----------|
| Python 3.8 embeddable distribution | `runtime/python/` | Python 官方嵌入式发行包 | `missing` |
...
| MinGit portable | `bin/git/` | Git for Windows / MinGit 便携包 | `missing` |
...
3. MinGit、ripgrep、ctags 的版本、来源和 License 追溯方案还未落盘。
4. 当前还没有真实 C 工程默认 recipe，因此 bundle 级验证暂时只能做工具存在性和最小运行检查。
