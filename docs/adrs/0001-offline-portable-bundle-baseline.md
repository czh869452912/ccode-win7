# ADR-0001: 离线交付采用 one-folder portable bundle 基线
...
6. 项目 Python 代码以六个独立、已检查的 wheel 进入离线 staging：Core、
   Protocol、Host、Composition、C/C++ Workflow 和产品聚合包
8. dependency export 只从 clean wheelhouse 安装项目 distribution，禁止把
7. 五个下层 distribution 安装到 `runtime/site-packages`，产品只暂存到
   `app/embedagent`，不在 runtime 中重复产品 package/dist-info
   editable 开发树或未经检查的源码目录作为 bundle 输入
9. 第三方 Python 依赖是独立、受控的构建时步骤，可构建 lock 固定的
   sdist；六个项目 distribution 自身必须只从已检查 wheel 安装
...
- 需要同步维护六个 distribution 的 metadata、wheel 边界和隔离导入门禁
...
wheel dependency 检查证明六个项目 distribution 之间的 exact DAG，不代表
对所有第三方依赖版本的完整审计。隔离安装场景覆盖六个项目 distribution
...
的 binary wheel 来源、license 与 hash 是 release hardening。构建结果进入
bundle 后，目标 runtime 仍不得访问网络或编译依赖。
