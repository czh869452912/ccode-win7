...
```bash
INIT=$(node "D:/Claude-project/ccode-win7/.opencode/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "D:/Claude-project/ccode-win7/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "D:/Claude-project/ccode-win7/.opencode/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
