...
Do not treat blueprint target terms such as `AgentKernel`, `SessionLog`, or `HookBus` as implemented public APIs until a specific implementation slice lands and updates the source-of-truth docs. `AgentEventBus` is now the internal source-aware event/reducer boundary for migrated extension hooks only; context and tool-result reducers have moved first, while other hook families still migrate in later Phase B slices. Near-term work must preserve the hosted C/C++ product behavior while extracting those boundaries gradually.
