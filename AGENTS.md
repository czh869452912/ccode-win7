...
Local resource reload is a file discovery operation. `ToolRuntime.reload_resources()`, `InProcessAdapter.reload_resources(...)`, `/resources reload`, and `POST /api/sessions/{session_id}/resources/reload` refresh workspace-bound skills, prompts, and recipe JSON resources. Skills/prompts are surfaced as resources; `.embedagent/recipes/*.json` feeds the existing recipe contract. Agent Skills-style frontmatter (`name`, `description`, `disable-model-invocation`) controls skill metadata and model prompt listing. `/skill:<name> [args]` expands a workspace-bound skill Markdown file into a normal user turn; it does not bypass tools, permissions, or extension loading. Reload does not execute local Python code. `author_local_capability` writes local self-extension artifacts under `.embedagent` and reports next actions; it does not implicitly reload resources or enable/load project extensions.
