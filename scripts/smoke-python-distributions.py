...
SCENARIOS = (
    {
        ...
        "probe": (
            "import importlib.util\n"
            "from embedagent_core import Agent, AgentPorts, UserTurn\n"
            ...
            "        return []\n"
            "    def commit_observation(self, session_log, session, action, observation, "
            "current_mode, turn_id='', step_id='', message_id='', "
            "parent_message_id='', finished_at=''):\n"
            "        del session_log, session, action, current_mode, turn_id, step_id\n"
            "        del message_id, parent_message_id, finished_at\n"
            "        return observation\n"
            ...
            ")\n"
            "result = Agent.create(ports).open('smoke-core').submit(UserTurn("
            "'hello', stream=False))\n"
            "isolated = not any(importlib.util.find_spec(name) is not None "
            "for name in blocked)\n"
            "raise SystemExit(0 if result.final_text == 'done' and isolated else 1)\n"
        ),
    },
    {
        "name": "protocol_only",
